...
$row = App\Models\PageContent::find(30);
$posts = $row->value;

echo "BEFORE UPDATE:\n";
foreach ($posts as $p) {
    if (strpos($p['title'], 'Advantages') !== false) {
        echo "Found post, content length: " . strlen($p['content']) . "\n";
        echo "Contains proper lists: " . (strpos($p['content'], '<ul>') !== false ? 'YES' : 'NO') . "\n";
    }
}

$cleanHtml = '<p><strong>From Outsourcing to Co-Sourcing: How Australian Businesses Are Quietly Rebuilding Their Teams</strong></p>' .
             '<p></p>' .
             '<p>For years, outsourcing has carried a certain stigma.</p>' .
             '<p></p>' .
             '<ul>' .
...
// Read it back from DB to verify it was saved!
$rowReloaded = App\Models\PageContent::find(30);
$postsReloaded = $rowReloaded->value;

echo "AFTER UPDATE:\n";
foreach ($postsReloaded as $p) {
    if (strpos($p['title'], 'Advantages') !== false) {
        echo "Found post, content length: " . strlen($p['content']) . "\n";
        echo "Contains proper lists: " . (strpos($p['content'], '<ul>') !== false ? 'YES' : 'NO') . "\n";
    }
